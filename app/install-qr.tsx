...
const APPS=[
 {key:'shift',path:'/',name:'Pelham Shift',icon:'/icons/icon-192.png',note:'스케줄·근무표·메시지'},
 {key:'punch',path:'/attendance',name:'Punch In',icon:'/icons/punch-192.png',note:'출근·퇴근·휴게 찍기'},
] as const;
export default function InstallQr({team,compact=false}:{team?:string;compact?:boolean}){
 ...
 const [pick,setPick]=useState<'shift'|'punch'>('shift');
 ...
 const here=open&&window.location.pathname.startsWith('/attendance')?'punch':'shift';
 ...
 return <><button className={compact?'button toolbutton':'button'} title={t('앱 설치 QR')} aria-label={t('앱 설치 QR')} onClick={()=>{setPick(window.location.pathname.startsWith('/attendance')?'punch':'shift');setOpen(true)}}><QrCode size={17}/>{!compact&&' '+t('앱 설치 QR')}</button><Dialog open={open} onOpenChange={setOpen}><DialogContent className="shift-dialog install-dialog"><DialogTitle>{t('휴대폰에 앱 설치')}</DialogTitle><DialogDescription>{t('앱은 두 개입니다. 하나씩 따로 설치하면 홈 화면에 아이콘이 두 개 생깁니다.')}</DialogDescription>
