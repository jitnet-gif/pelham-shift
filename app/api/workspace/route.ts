...
if(body.type==='message'){const fresh=state.messages.slice(c.state.messages.length);const to=[...new Set(fresh.flatMap(m=>m.to==='all'?state.employees.map(e=>e.id):[m.to]))];after(()=>notify(c.team,to.filter(id=>id&&id!==c.actor.id),{title:c.actor.admin?'관리자 메시지':'직원 메시지',body:(c.actor.admin?'':(state.employees.find(e=>e.id===c.actor.id)?.name||'')+': ')+(fresh[0]?.body||''),tag:'message'},new URL(req.url).origin).catch(()=>0))}if(body.type==='notice'){const m=state.messages.at(-1);after(()=>notify(c.team,(m?.recipients??state.employees.filter(e=>!e.archived).map(e=>e.id)).filter(id=>id!==c.actor.id),{title:'관리자 공지',body:m?.body||'',tag:'notice'},new URL(req.url).origin).catch(()=>0))}if(body.type==='rain'){const m=state.messages.at(-1);after(()=>notify(c.team,m?.recipients??state.employees.map(e=>e.id),{title:'우천 근무 종료',body:m?.body||'',tag:'rain'},new URL(req.url).origin).catch(()=>0))}return json({state:visible(state,c.actor),version:c.row.version+1,team:c.team,actor:c.actor})}catch(e){return json({error:e instanceof Error?e.message:'저장하지 못했습니다.'},400)}}
