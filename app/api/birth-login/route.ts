...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as { birthDate?: unknown };
    const team = new URL(request.url).searchParams.get('team') || '';
    const session = await createBirthSession(String(body.birthDate || ''), team);
    return Response.json(
      { team: session.team, actor: session.actor },
