...
export function visible(state:State,actor:Actor){const tasks=state.tasks??[];if(actor.admin)return {...state,tasks};const taskManager=!!state.employees.find(e=>e.id===actor.id)?.taskManager;return {...state,tasks:taskManager?tasks:tasks.filter(t=>t.assignedTo===actor.id),employees:state.employees.map(e=>e.id===actor.id?{...e,email:'',birthDate:'',phone:''}:{...e,email:'',birthDate:'',phone:'',rate:0}),shifts:state.published?state.shifts:[],attendance:state.attendance.filter(a=>a.employeeId===actor.id),swaps:[],
