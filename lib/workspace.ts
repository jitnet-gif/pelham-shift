...
export function visible(state:State,actor:Actor){const tasks=state.tasks??[];if(actor.admin)return {...state,tasks};return {...state,tasks:[],employees:state.employees.map(e=>e.id===actor.id?e:{...e,email:'',rate:0}),shifts:state.published?state.shifts:[],attendance:state.attendance.filter(a=>a.employeeId===actor.id),swaps:[],messages:[]}}
