...
import {type Shift,type State,OT_WEEKLY_HOURS,dueShifts,missedShifts,overtimeWorked,payPeriodStart} from './domain';
...
export async function remind(workspace:string,state:State,origin:string,now=new Date()){let sent=0;
 ...
 const admins=[...rosterAdminIds(),...state.employees.filter(e=>e.admin&&!e.archived).map(e=>e.id)];
