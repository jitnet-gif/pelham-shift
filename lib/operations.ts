...
export function applyCommand(current:State,command:Command,actor:Actor,now=new Date()):State{
 const s=structuredClone(current),p=command.payload??{},id=()=>crypto.randomUUID();s.tasks??=[];s.timeOff??=[];s.availability??=[];s.punches??=[];s.clockNames??=[];
 // 공개본이 아직 없던 워크스페이스는 지금 직원이 보고 있는 모습을 첫 공개본으로 굳힙니다.
 s.publishedShifts??=structuredClone(publicShifts(s));
 const admin=()=>{if(!actor.admin)fail('관리자 권한이 필요합니다.')};const employee=(v:string)=>s.employees.find(e=>e.id===v)??fail('등록된 직원을 선택하세요.');
 ...
 // 지운 근무는 작업본에서만 빠집니다. 직원 화면에서는 다음 공개 때 사라집니다.
 ...
 // 공개하면 모든 근무의 Unpublished 딱지를 떼고, 지금 작업본을 새 공개본으로 떠 둡니다.
 // 공개를 거두는 명령은 두지 않습니다 — 직원 화면은 언제나 마지막 공개본을 보여 줍니다.
 case 'publish':scheduler();for(const x of s.shifts)delete x.draft;s.publishedShifts=structuredClone(s.shifts);break;
 ...
 case 'message': {const targets=[...new Set(String(p.to??'').split(',').map(v=>v.trim()).filter(Boolean))];if(!targets.length)fail('받는 사람을 선택하세요.');if(targets.length>300)fail('한 번에 300명까지 보낼 수 있습니다.');if(!actor.admin&&targets.length>1)fail('여러 명에게 보내기는 관리자만 할 수 있습니다.');const body=text(p.body,2000);for(const to of targets){if(!actor.admin&&(to==='all'||to===actor.id))fail('직원은 관리자나 동료에게만 메시지를 보낼 수 있습니다.');if(to!=='admin'&&to!=='all'){const who=employee(to);if(!actor.admin&&who.archived)fail('퇴사한 직원에게는 보낼 수 없습니다.');}if(to==='all')admin();s.messages.push({id:id(),sender:actor.id,to,body,createdAt:now.toISOString(),readBy:[actor.id],kind:'message'});}break;}
 // 공지는 공지 탭에 한 건으로 올라갑니다. 일부만 고르면 그 사람들에게만 보입니다.
 case 'notice': {admin();const ids=[...new Set(String(p.to??'').split(',').map(v=>v.trim()).filter(Boolean))];if(!ids.length)fail('공지를 받을 직원을 선택하세요.');for(const v of ids)if(employee(v).archived)fail('퇴사한 직원에게는 보낼 수 없습니다.');const all=s.employees.filter(e=>!e.archived).every(e=>ids.includes(e.id));s.messages.push({id:id(),sender:actor.id,to:'all',...(all?{}:{recipients:ids}),body:text(p.body,2000),createdAt:now.toISOString(),readBy:[actor.id],kind:'notice'});break;}
