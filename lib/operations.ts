...
export function applyCommand(current:State,command:Command,actor:Actor,now=new Date()):State{
 ...
 case 'message': {if(!actor.admin&&(p.to==='all'||p.to===actor.id))fail('직원은 관리자나 동료에게만 메시지를 보낼 수 있습니다.');if(p.to!=='admin'&&p.to!=='all'){const to=employee(p.to);if(!actor.admin&&to.archived)fail('퇴사한 직원에게는 보낼 수 없습니다.');}if(p.to==='all')admin();s.messages.push({id:id(),sender:actor.id,to:p.to,body:text(p.body,2000),createdAt:now.toISOString(),readBy:[actor.id],kind:'message'});break;}
